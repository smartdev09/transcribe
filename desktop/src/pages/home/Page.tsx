...
//import { File, Mic, Link } from 'lucide-react'
	...
	return (
		<Layout>
			...
			<button
				...
				className={cx(
					'ml-2 px-3 py-2 rounded-md transition-all duration-200',
					'border border-gray-500 bg-zinc-700 hover:bg-zinc-600',
					vm.preference.homeTabIndex === 0 && 'bg-blue-600 text-white'
				)}
			>
				<Mic className="w-5 h-5" /> New Recording
			</button>
			
			<div role="tablist" className="tabs tabs-lifted flex justify-between w-full max-w-4xl mx-auto mt-5">
				<a
					role="tab"
					onClick={() => vm.preference.setHomeTabIndex(0)}
					className={cx('tab flex-1 justify-center [--tab-border-color:gray]', vm.preference.homeTabIndex === 0 && 'tab-active')}>
					<Mic className="w-5 h-5" />
				</a>
				<a
					role="tab"
					onClick={() => vm.preference.setHomeTabIndex(1)}
					className={cx('tab flex-1 justify-center [--tab-border-color:gray]', vm.preference.homeTabIndex === 1 && 'tab-active')}>
					<File className="w-5 h-5" />
				</a>
				<a
					role="tab"
					onClick={vm.switchToLinkTab}
					className={cx('tab flex-1 justify-center [--tab-border-color:gray]', vm.preference.homeTabIndex === 2 && 'tab-active')}>
			...
			{/* Record */}
			{
				vm.preference.homeTabIndex === 0 && (
					<>
						<div className="flex w-full max-w-4xl mx-auto flex-col px-4">
							...
							{!vm.isRecording && (
								<div className='text-center'>
									<button onMouseDown={() => vm.startRecord()} className="btn btn-primary mt-3 bg-purple-600 hover:bg-purple-700 w-80">
								...
								<>
									<div className="text-center">
										<button
											...
											className="btn relative btn-success mt-3 bg-purple-600 hover:bg-purple-700 w-80">
			...
			{/* File */}
			{
				vm.preference.homeTabIndex === 1 && (
					<>
						<div className="flex w-full max-w-4xl mx-auto flex-col mt-5 px-4">
							...
							{vm.audio && !vm.loading && (
								<>
									<div className='text-center'>
										<button onMouseDown={() => vm.transcribe(vm.files[0].path)} className="btn btn-primary mt-3 bg-purple-600 w-50 hover:bg-purple-700 w-80">
											{t('common.transcribe')}
										</button>
									</div>
									{/*	<ModelOptions options={vm.preference.modelOptions} setOptions={vm.preference.setModelOptions} />
						...
						<div className="flex flex-col gap-0 mt-5">
							...
							{vm.downloadingAudio ? (
								...
								<>
									...
									<button onMouseDown={vm.downloadAudio} className="btn btn-primary mt-0 bg-purple-600 hover:bg-purple-700">
